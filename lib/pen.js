...
export default class Painter {
  ...
  _doClip(borderRadius, width, height, borderStyle) {
    if (borderRadius && width && height) {
      ...
      if (!(getGlobal().systemInfo &&
          getGlobal().systemInfo.version <= '6.6.6' &&
          getGlobal().systemInfo.platform === 'ios')) {
        this.ctx.clip();
      }
  ...
  _preProcess(view, notClip) {
    ...
    switch (view.type) {
      ...
      case 'image': {
        ...
        const ratio = getGlobal().systemInfo.pixelRatio ? getGlobal().systemInfo.pixelRatio : 2;
